...
def create_payment(
  ...
  expense_sheets = create_expense_sheets(state: previous_state, service:)

  # saving issues state update to requested state
  Payment.new(expense_sheets:, state:, payment_timestamp:).tap(&:save)
end
...
def create_service(user)
  ...
  create :service, :long, beginning:, ending:, user:
end
