...
RSpec.describe V1::ServiceCalculatorController, type: :request do
  ...
  let(:service_days_request) { get calculate_service_days_v1_services_path(beginning:, ending:) }
  let(:ending) { '2018-01-26' }
  let(:ending_request) { get calculate_ending_v1_services_path(beginning:, service_days:) }
  let(:service_days) { 26 }
